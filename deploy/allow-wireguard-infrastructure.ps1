...
$forwardPorts = [string[]]@('6379', '5672', '9092', '15672')
...
foreach ($port in $forwardPorts) {
    ...
    & netsh.exe interface portproxy add v4tov4 listenaddress=10.66.0.2 listenport=$port connectaddress=127.0.0.1 connectport=$port protocol=tcp | Out-Null

    if ($LASTEXITCODE -ne 0) {
        throw "创建端口代理失败: 10.66.0.2:$port -> 127.0.0.1:$port"
    }
}

Write-Host "已允许 WireGuard 接口 chat-web-home 访问端口: $($ports -join ', ')"
Write-Host "已创建 Docker Desktop 端口代理: $($forwardPorts -join ', ')"
